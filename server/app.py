...
async def websocket_handler(request):
    ...
    connected_websockets[username] = ws

    await broadcast({"type": "online", "users": list(online_users)})
    ...
    async for msg in ws:
        if msg.type == web.WSMsgType.TEXT:
            ...
            if data["type"] == "typing":
                await broadcast({"type": "typing", "user": username})
                ...
                await broadcast({"type": "read", "user": username})

    online_users.remove(username)
    del connected_websockets[username]
    await broadcast({"type": "online", "users": list(online_users)})
...
async def broadcast(data):
    dead = []
    for user, ws in connected_websockets.items():
        try:
